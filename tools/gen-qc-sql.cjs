...
const __FINISH_INSPECT_TODAY = today
const TODAY = today
const clean = (t) => t.replace(/\/\/[^\n]*/g, '').replace(/\/\*[\s\S]*?\*\//g, '')
  .replace(/const TODAY = \(typeof today !== 'undefined'\) \? today : [^\n]*/g, 'TODAY')
...
const PANELS = [
  ['ARRIVAL_IN', '\u5230\u8d27\u5355'],
  ['FINISH_INSPECT', '\u6210\u54c1\u62a5\u68c0\u5355'],
  ['INSPECTION', '\u6765\u6599\u6210\u54c1\u68c0\u9a8c\u5355'],
  ['DISPATCH', '\u5de5\u5e8f\u6d3e\u5de5\u5355'],
]
let sql = '-- QC \u9009\u5355\u6e90\u9762\u677f\u540c\u6b65\uff082026-08-20\uff09\n'
for (const [code, name] of PANELS) {
  ...
  if (cfgStart < 0 || cfgEnd < 0) { console.error(code + ' anchors missing'); continue }
  ...
  let cfg, rows
  try {
    cfg = eval('(' + clean(cfgText) + ')')
    rows = eval(clean(rowsText))
  } catch (e) { console.error(code + ' eval failed: ' + e.message); continue }
  const c2 = inject(cfg)
  ...
  console.log(code + ': ' + (cfg.metadata ? cfg.metadata.panelName : '?') + ' ok, rows=' + (rows ? rows.length : 0))
}
fs.writeFileSync('F:/INCER/light-mes/tools/update-qc.sql', sql)
