...
async function main() {
  ...
  for (const code of PANELS) {
    ...
    const r = await evalMain(`(() => {
      ...
      const dup = mains.filter((m, i) => mains.indexOf(m) !== i);
      return JSON.stringify({ mains, dup: [...new Set(dup)] });
    })()`)
    let o = {}
    try { o = JSON.parse(r) } catch (e) { o = { mains: [], dup: [] } }
    console.log(`[${code}] ${o.mains.join('/')} ${o.dup.length ? '!!重复: ' + o.dup.join(',') : ''}`)
  }
  ws.close()
}
