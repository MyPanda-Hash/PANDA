...
async function main() {
  ...
  for (const code of PANELS) {
    ...
    const r = await evalMain(`(() => {
      ...
      return JSON.stringify({ count: mains.length, dup });
    })()`)
    let o = {}
    try { o = JSON.parse(r) } catch (e) { o = { count: 0, dup: ['scan-err'] } }
    if (o.dup.length) allOk = false
    console.log(`[${code}] 按钮数=${o.count} ${o.dup.length ? '!!重复: ' + o.dup.join(',') : 'OK'}`)
  }
