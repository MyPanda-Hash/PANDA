...
### 21. 灰按钮点击穿透：草稿态点「生成销售出库单(普通销售)」仍弹提示（视觉置灰不拦事件）

- **现象**：草稿销售订单下「生单」组（主按钮=生成销售出库单(普通销售)）显示灰色，点击仍触发后端调用并弹出「仅已审核销售订单可生成销售出库单」等提示词；下拉项「生成销售出库单(销售退货)」同样可点。用户判定为 bug：灰按钮点击不应有任何反应。
- **根因**：`disabled` 只是渲染层视觉 class（`.tb-main.disabled` / `.ctx-item`），`@click` 事件绑定未解除；`onButton`/`onGroupAction` 入口无 `isDisabled` 前置拦截，事件穿透到 `engine.callButton` → 后端校验抛错 → `ElMessage` 提示。此前 isDisabled 仅用于置灰展示，未作为执行闸门。
- **修复**：`PanelxList.vue` / `PanelxForm.vue` 的 `onButton` 与 `onGroupAction` 第一行统一 `if (isDisabled(action)) return`（不执行、不提示、零后端请求）；下拉项 `.ctx-item` 增加 `disabled` class 视觉（灰字 + not-allowed）。所有「生成XX」按钮统一进 isDisabled 状态矩阵（仅 已审核/生产中 可用）。
- **验证**：CDP 实测草稿 SO 点「生单」主按钮 → 无提示词、无弹窗、页面不跳转、`callButton` 网络请求 0 次；已审核单据按钮恢复可用且生单成功。
- **教训**：**按钮可用性=isDisabled 状态矩阵 + 入口事件拦截双保险**；新增任何按钮动作必须同时进 isDisabled 与 onButton 拦截，禁止只做视觉置灰。联动「已审核才能选择生单」四层保障见 开发规范与需求总览 §21 / 面板交互设计规范 §10.2。
